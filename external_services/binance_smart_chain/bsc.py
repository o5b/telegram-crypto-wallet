...
bsc_client.middleware_onion.inject(async_geth_poa_middleware, layer=0)
...
async def get_bnb_balance(wallet_addresses, client):
    ...
    try:
        ...
        if isinstance(wallet_addresses, str):
            balance = await bsc_client.eth.get_balance(w3.to_checksum_address(wallet_addresses))
        ...
        # Если передан список адресов кошельков
            ...
            for address in wallet_addresses:
                balance = await bsc_client.eth.get_balance(w3.to_checksum_address(address))
                # Преобразование wei в BNB
                bnb_balance = balance / WEI_TO_BNB_RATIO
...
async def get_bsc_transaction_history(wallet_address: str, blocknumber_start: int, blocknumber_end: int) -> list[dict] | None:
    """
        Retrieves transaction history for a given Binance Smart Chain (BSC) wallet address.
    ...
    try:
        ...
        for blocknumber in range(blocknumber_start, blocknumber_end):
            print(f'blocknumber: {blocknumber}')
            block = await bsc_client.eth.get_block(block_identifier=blocknumber, full_transactions=True)

            for transaction in block.transactions:

                print(f"transaction['from']: {transaction['from']}")
                print(f"transaction['to']: {transaction['to']}")

                if transaction:

                    if hasattr(transaction, 'to') and isinstance(transaction['to'], str):
                        if transaction['to'].lower() == wallet_address.lower():
                            transaction_history.append(transaction)
                            print(f'block: {block}, address: {wallet_address}')

                    if hasattr(transaction, 'from') and isinstance(transaction['from'], str):
                        if transaction['from'].lower() == wallet_address.lower():
                            if transaction not in transaction_history:
                                transaction_history.append(transaction)
                                print(f'block: {block}, address: {wallet_address}')

        print(f'len transaction_history: {len(transaction_history)}')
        return transaction_history

    except Exception as e:
        detailed_error_traceback = traceback.format_exc()
        logger.error(f"Failed to get transaction history for BSC wallet {wallet_address}: {e}\n{detailed_error_traceback}")
        return []
