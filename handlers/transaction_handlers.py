...
from external_services.solana.solana import get_solana_transaction_history
from external_services.binance_smart_chain.bsc import get_bsc_transaction_history, bsc_client
    ...
    try:
        ...
        while transaction_max_limit > 0:
            ...
            if CURRENT_BLOCKCHAIN == 'bsc':
                checksum_address = bsc_client.to_checksum_address(wallet_address)
                transaction_count =  await bsc_client.eth.get_transaction_count(checksum_address)
                wallet_address_format = f'{wallet_address[:6]}***{wallet_address[-4:]}'
                bsc_transaction_count = f'Transaction count: <b>{transaction_count}</b> \nfor wallet address: {wallet_address_format}'
                print(transaction_count)
                # getting the transaction history from Binance Smart Chain takes too long,
                # since you need to look for the wallet address in each block in each transaction

                # # request the latest block number
                # blocknumber_end = await bsc_client.eth.get_block_number()
                # blocknumber_start = blocknumber_end - 10
                # print(f'latest block number: {blocknumber_end:_}')
                # transaction_history += await get_bsc_transaction_history(wallet_address, blocknumber_start, blocknumber_end)
        ...
        elif bsc_transaction_count:
            await callback.message.answer(bsc_transaction_count)
